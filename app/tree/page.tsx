...
const Tree: FC = () => {
  ...
  const inputHandler = useCallback((event: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setList(
      value
        ...
        .replace(/,$/, '')
        .split(',')
        .map((i) => (isNaN(parseInt(i.trim())) ? 'n' : parseInt(i.trim()))),
    )
  }, [])
  ...
  return (
    <main className="w-screen h-screen flex flex-col justify-start items-center bg-slate-200">
      ...
      <div>
        {nodeList.map((list, index) => (
          <div className="flex justify-evenly items-center min-w-[50rem] py-2" key={index}>
            {list.map((e, eIndex) => (
              <span
                className="border border-solid border-black text-black w-8 h-8 flex justify-center items-center rounded-full"
                key={`${index}-${e}-${eIndex}`}
              >
                {e}
              </span>
